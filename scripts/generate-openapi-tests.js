...
const operationSelectionPath = path.join(root, 'nodes/Dokaai/operation-selection.ts');
...
const {
	...
	LEGACY_DEVICE_TOKEN_OPERATION_IDS,
} = require(path.join(root, 'nodes/Dokaai/shared/legacy-device-token-fields'));
...
const buildFixture = (operationId, resource) => {
	...
	const legacyDeviceTokenFields = LEGACY_DEVICE_TOKEN_OPERATION_IDS.has(operationId)
		? collectBodyFields(legacyDeviceTokenSchema, new Set())
		: [];
	const allBodyFields = [...bodyFields, ...legacyDeviceTokenFields];
	const inputData = {};
	...
	for (const field of allBodyFields) {
		inputData[field.name] = n8nInputValue(field.name, field.schema);
	}

	let expectedBody = buildExpectedBody(allBodyFields, inputData, bodyRoot);
	...
	return {
		...
		bodyFieldNames: allBodyFields.map((field) => field.name),
...
const resourceGroups = groupOperationIdsByFirstTag(spec, selectedOperationIds);
const operationsByResource = Object.fromEntries(
	resourceGroups.map((resource) => [resource.value, resource.operationIds]),
);
const fixtures = resourceGroups.flatMap((resource) =>
	resource.operationIds.map((operationId) => buildFixture(operationId, resource.value)),
);

const testSource = `// Auto-generated by npm run generate:tests.
// Do not edit directly; update api/index.json, nodes/Dokaai/operation-selection.ts, or n8n compatibility patches.
...
const resourceGroups = ${JSON.stringify(resourceGroups, null, 2)};
